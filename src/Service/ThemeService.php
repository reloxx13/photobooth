...
class ThemeService
{
    ...
    public function getAll(): array
    {
        ...
        // New location: private/themes/*.theme.config.json
        $themes = $this->loadThemesFromDirectory($this->themeDirectory, $themes);

        // Older per-file location: config/themes/*.theme.config.json
        $themes = $this->loadThemesFromDirectory($this->legacyThemeDirectory, $themes);

        // Backwards compatibility: load themes from legacy JSON file if present.
        $legacyFile = PathUtility::getAbsolutePath('private/themes.json');
        if (is_file($legacyFile)) {
            $legacyRaw = @file_get_contents($legacyFile);
            if ($legacyRaw !== false) {
                $legacyDecoded = json_decode($legacyRaw, true);
                if (is_array($legacyDecoded)) {
                    foreach ($legacyDecoded as $name => $data) {
